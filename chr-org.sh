...
while true; do
    ...
    case $version_choice in
        1) VERSION=$(curl -s "https://upgrade.mikrotik.com/routeros/NEWESTa7.stable" | cut -d' ' -f1); V7=1 ;;
        2) VERSION=$(curl -s "https://upgrade.mikrotik.com/routeros/NEWESTa7.testing" | cut -d' ' -f1); V7=1 ;;
        3)
            if [[ "$ARCH" == "aarch64" ]]; then
                echo "$MSG_ARM64_NOT_SUPPORT_V6"
                continue
            fi
            VERSION=$(curl -s "https://upgrade.mikrotik.com/routeros/NEWEST6.long-term" | cut -d' ' -f1)
            V7=0
            ;;
        4)
            if [[ "$ARCH" == "aarch64" ]]; then
            ...
            VERSION=$(curl -s "https://upgrade.mikrotik.com/routeros/NEWEST6.stable" | cut -d' ' -f1)
            V7=0
            ;;
        *)
...
case $ARCH in
    x86_64|i386|i486|i586|i686)
        if [[ $V7 == 1 && $BOOT_MODE == "BIOS" ]]; then
            IMG_URL="https://download.mikrotik.com/routeros/$VERSION/chr-$VERSION.img.zip"
        else
            IMG_URL="https://download.mikrotik.com/routeros/$VERSION/chr-$VERSION.img.zip"
        fi
        ;; 
    aarch64)
        IMG_URL="https://download.mikrotik.com/routeros/$VERSION-arm64/chr-$VERSION-arm64.img.zip"
        ;; 
    *)
